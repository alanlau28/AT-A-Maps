...
SUITE(street_queries_perf_public_toronto_canada) {
    ...
    TEST_FIXTURE(MapFixture, bounding_box_of_street_perf) {
        //Verify Functionality
        LatLonBounds actual;
        ...
        {
            //Timed Test
            ECE297_TIME_CONSTRAINT(289);
            LatLonBounds result;
            for(size_t i = 0; i < 100000; i++) {
                result = findStreetBoundingBox(street_ids[i]);
            }
        }
    } //bounding_box_of_street_perf
